...
//     console.log(hospitalData?.data, "hii");
    

//   const [updateBooking, { isLoading: isUpdating }] = useUpdatebookingMutation()
//     const [updateHospital, { isLoading: isUpdatingHospital }] = useUpdateHospitalMutation()
...
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { Search, Calendar, Filter, ChevronLeft, ChevronRight, Clock, Settings } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function BookingsPage() {
  ...
  // Initialize auto decline minutes from hospital data
  useEffect(() => {
    if (hospitalData?.data?.autoDeclineMinutes !== undefined) {
      setAutoDeclineMinutes(hospitalData.data.autoDeclineMinutes)
    }
  }, [hospitalData])
  ...
  // Handle auto decline minutes update
  const handleAutoDeclineUpdate = async () => {
    try {
      setIsUpdatingSettings(true)
      
      await updateHospital({
        id: hospitalId,
        data: { autoDeclineMinutes }
      }).unwrap()
      
      toast.success("Auto decline settings updated successfully!")
      setIsSettingsDialogOpen(false)
      hospitalRefetch()
    } catch (error) {
      const msg = error?.data?.message || "Failed to update settings";
      toast.error(msg);
    } finally {
  ...
  const calculateRemainingTime = (createdAt) => {
    if (!hospitalData?.data?.autoDeclineMinutes) return null
    
    const createdTime = new Date(createdAt).getTime()
    const currentTime = new Date().getTime()
    const elapsedMinutes = Math.floor((currentTime - createdTime) / (1000 * 60))
    const remainingMinutes = hospitalData.data.autoDeclineMinutes - elapsedMinutes
  ...
  // Pending bookings with countdown
  const pendingBookingsWithTimer = useMemo(() => {
    if (!bookingData?.data) return []
    ...
    return bookingData.data
      ...
      .map(booking => ({
        ...booking,
        remainingMinutes: calculateRemainingTime(booking.createdAt)
      }))
  }, [bookingData, hospitalData])
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      ...
      {/* Auto Decline Settings Dialog */}
      <Dialog open={isSettingsDialogOpen} onOpenChange={setIsSettingsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Auto Decline Settings</DialogTitle>
            <DialogDescription>
              Set the automatic decline time for pending bookings. Bookings not accepted within this time will be automatically declined.
            </DialogDescription>
          </DialogHeader>
          
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="auto-decline-minutes">Auto Decline Time (Minutes)</Label>
              <Input
                id="auto-decline-minutes"
                type="number"
                min="1"
                max="1440"
                value={autoDeclineMinutes}
                onChange={(e) => setAutoDeclineMinutes(parseInt(e.target.value) || 5)}
                className="w-full"
              />
              <p className="text-xs text-muted-foreground">
                Set how many minutes a booking stays in pending status before being automatically declined (1-1440 minutes)
              </p>
            </div>
            ...
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <p className="text-sm text-blue-700 mt-1">
                Pending bookings will be automatically declined after <strong>{autoDeclineMinutes} minutes</strong> if not accepted.
              </p>
              {autoDeclineMinutes >= 60 && (
                <p className="text-sm text-blue-600 mt-1">
                  ({Math.floor(autoDeclineMinutes / 60)} hour{Math.floor(autoDeclineMinutes / 60) > 1 ? 's' : ''} 
                  {autoDeclineMinutes % 60 > 0 ? ` and ${autoDeclineMinutes % 60} minute${autoDeclineMinutes % 60 > 1 ? 's' : ''}` : ''})
                </p>
              )}
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleAutoDeclineUpdate}
              disabled={isUpdatingSettings || autoDeclineMinutes < 1 || autoDeclineMinutes > 1440}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          <div className="flex gap-2">
            ...
            <Button 
              className="whitespace-nowrap bg-green-600 hover:bg-green-700 cursor-pointer" 
              onClick={() => Navigate("/dashboard/bookings/doctor-mange")}
            >
              Manage Doctor Booking
            </Button>
          </div>
      ...
      {/* Auto Decline Info Banner */}
      {hospitalData?.data?.autoDeclineMinutes && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-blue-800">
                  Auto Decline: {hospitalData.data.autoDeclineMinutes} minutes
                </p>
                <p className="text-xs text-blue-600">
                  Pending bookings will be automatically declined after {hospitalData.data.autoDeclineMinutes} minutes
                  {pendingBookingsWithTimer.length > 0 && ` • ${pendingBookingsWithTimer.length} pending booking(s) with countdown`}
                </p>
              </div>
            </div>
            <Button
              ...
              <div className="flex-1 overflow-auto">
                <div className="min-w-[1000px]">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {paginatedBookings.map((booking) => {
                        const remainingMinutes = booking.status === 'pending' ? calculateRemainingTime(booking.createdAt) : null
                              ...
                              {remainingMinutes !== null && remainingMinutes > 0 && (
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  <span className="text-xs text-orange-600 font-medium">
                                    Auto decline in: {remainingMinutes}m
                                  </span>
                                </div>
                              )}
                              {remainingMinutes !== null && remainingMinutes <= 0 && (
