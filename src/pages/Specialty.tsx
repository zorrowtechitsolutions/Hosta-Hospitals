...
const SpecialtyForm: React.FC<SpecialtyFormProps> = ({
  ...
  const handleSpecialtyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedSpecialty(value);
    setShowSuggestions(true);
    setSpecialtyError(""); // Clear error when user types
  };
  ...
  const handleSuggestionClick = (suggestion: string) => {
    ...
    setShowSuggestions(false);
    setSpecialtyError(""); // Clear error when selection is made
  };
  ...
  // Check if selected specialty is valid (exists in suggestions)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!specialty) {
      ...
      if (!isValidSpecialty()) {
        setSpecialtyError("Please select a valid specialty from the list");
        return;
      }
    }
      ...
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-2 mt-6 pt-4 border-t border-green-200">
            ...
            <button
              type="submit"
              disabled={!!phoneError || (!!specialtyError && !specialty)} // Disable submit if phone validation fails or specialty is invalid
              className={`px-4 py-2 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 ${
                phoneError || (specialtyError && !specialty)
                  ? 'bg-gray-400 text-gray-700 cursor-not-allowed'
                  : 'bg-green-600 text-white hover:bg-green-700'
              }`}
            >
